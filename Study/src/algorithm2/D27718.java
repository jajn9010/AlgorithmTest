...
public class D27718 {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < N; i++) {
			...
			l[i] = new Line(Long.parseLong(st.nextToken()), Long.parseLong(st.nextToken()),
	...
	static int isCrossing(Point p1, Point p2, Point p3, Point p4) {
		boolean crossing;
		int product1 = outPdctDirection(p1, p2, p3) * outPdctDirection(p1, p2, p4),
				product2 = outPdctDirection(p3, p4, p1) * outPdctDirection(p3, p4, p2);
		if (product1 == 0 && product2 == 0) {
			crossing = p1.compareTo(p4) <= 0 && p2.compareTo(p3) >= 0;
		} else {
			crossing = product1 <= 0 && product2 <= 0;
		}
		if (!crossing)
			return 0;
		if ((p1.x - p2.x) * (p3.y - p4.y) - (p1.y - p2.y) * (p3.x - p4.x) != 0
				|| (p1.compareTo(p4) == 0 && p3.compareTo(p1) <= 0)
				|| (p2.compareTo(p3) == 0 && p1.compareTo(p3) <= 0)) {
